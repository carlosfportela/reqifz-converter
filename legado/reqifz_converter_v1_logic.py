...
class ReqIFZConverterV1:
    def __init__(self, input_file, output_file):
        self.input_file = input_file
        self.output_file = output_file
        self.logger = logging.getLogger(__name__)

    def patch_reqif_xml(self, xml_path, file_map):
        parser = etree.XMLParser(recover=True, remove_blank_text=True)
        try:
            tree = etree.parse(xml_path, parser)
            root = tree.getroot()
        except Exception as e:
            self.logger.error(f"  [Erro] Falha ao processar {xml_path}: {e}")
            return
    ...
    def convert(self):
        temp_dir = tempfile.mkdtemp(prefix="migration_v4_idfix_")
        try:
            self.logger.info(f"Iniciando conversão (algoritmo v1) de {self.input_file}")
            ...
            for r, d, f in os.walk(temp_dir):
                for file in f:
                    if file.lower().endswith(('.reqif', '.xml')):
                        self.patch_reqif_xml(os.path.join(r, file), file_map)
                        self.logger.info(f"Processado XML via v1: {file}")
            with zipfile.ZipFile(self.output_file, 'w', zipfile.ZIP_DEFLATED) as z_out:
                for r, d, f in os.walk(temp_dir):
                    for file in f:
                        fp = os.path.join(r, file)
                        z_out.write(fp, os.path.relpath(fp, temp_dir))
            self.logger.info(f"[OK] Conversão v1 concluída. Arquivo gerado: {self.output_file}")
        finally:
